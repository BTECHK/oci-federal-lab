...
> **2026-05-07 RESTRUCTURE BANNER (read first):** This document is the **original** master design from March 2026. The cost-conscious restructure dated 2026-05-07 keeps most of it intact and changes only three things: (1) the **CloudBees CI migration arc is dropped** — distinctive features are replicated on free OSS Jenkins plugins, see **ADR-010**; (2) the **Phase 2 AI pillar swaps from a direct OCI Generative AI Service API call to OCI Generative AI Agents (managed RAG)** for incident triage; (3) the **budget framing changes** to a $150 hard cap with ~$70 discretionary, ~$30 reserve (trial credits no longer in play). The Phase 2 k3s plan and the Phase 3 Helm/ArgoCD plan are **preserved** — see **ADR-011** (which supersedes a same-day-walked-back ADR-009 that had briefly proposed pivoting Phase 2 to OKE Basic; that pivot would have collapsed the deliberate "primitives first, managed second" learning progression). Where the original text below conflicts with ADR-010, ADR-011, or `C:\Users\k_a_s\.claude\plans\i-think-for-phase-abundant-pixel.md`, the restructure plan and the ADRs win. Original design preserved here for historical context and traceability.
